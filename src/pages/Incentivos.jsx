  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-400">Los 6 incentivos activos del período. Cambia el mes arriba para ver puntos y premios de cada periodo.</p>
            ...
            <Card key={e.incentivoId}>
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg font-semibold text-white">{inc.nombre}</h2>
              ...
              <p className="text-xs text-slate-500 mb-4">{inc.descripcion}</p>

              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="bg-bg-surface2 rounded-lg p-3 border border-bg-border">
                  <p className="text-xs text-slate-500">{e.mecanica === 'directo' ? 'GP Coins directos' : 'Puntos acumulados'}</p>
                  <p className="text-xl font-semibold tabnum text-white">
                    {e.mecanica === 'directo' ? fmtNum(e.gp) : fmtNum(e.puntos)}
                    {e.mecanica === 'mixta' && <span className="text-sm text-gp-gold"> · {fmtNum(e.gp)} GP</span>}
                  </p>
                </div>
                <div className="bg-bg-surface2 rounded-lg p-3 border border-bg-border">
                  <p className="text-xs text-slate-500">Llaves cumplidas</p>
                  <p className="text-xl font-semibold tabnum text-white">{cumplidas}/{e.llaves.length}</p>
                </div>
              </div>
